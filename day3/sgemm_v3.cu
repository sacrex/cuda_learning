...
// block-tiling + thread-tiling + share_memory + solve bank conflict + share memory dobule buffing
__global__ void sgemm_v3(float * __restrict__ a, float * __restrict__ b,
    ...
    //1. 第一次读取global数据到share memory(寄存器做中转)
    {
        int load_a_gmem_k = 0 * BK + load_a_smem_k; // bk = 0, 从矩阵A中读取的列
        ...
        int load_b_gmem_k = 0 * BK + load_b_smem_k;  // bk = 0, 从矩阵B中读取的行
    ...
    for (int bk = 1; bk < (K + BK - 1) / BK; ++bk) {
        int smem_loc = (bk - 1) & 1;
        int smem_loc_next = bk & 1;

        // 3. 读取第二份global数据到寄存器，先不存储到share memroy.
        ...
        // 4. 用第一次读取的share memory中的数据进行计算
        // thread tiling
        // 每个线程从s_a和s_b中取数据,计算自己的TM*TN的矩阵
        #pragma unroll
        for (int k = 0; k < BK; ++k) {
            //每个线程读取从s_a和s_b分别读取8个元素，为了解决bank conflict,读取的2个4元素，相隔一半读取。
            FLOAT4(r_comp_a[0]) = FLOAT4(s_a[smem_loc][k][ty * TM / 2]);
            FLOAT4(r_comp_a[4]) = FLOAT4(s_a[smem_loc][k][ty * TM / 2 + BM / 2]);
            FLOAT4(r_comp_b[0]) = FLOAT4(s_b[smem_loc][k][tx * TN / 2]);
            FLOAT4(r_comp_b[4]) = FLOAT4(s_b[smem_loc][k][tx * TN / 2 + BN / 2]);
        ...
        // 5.用第二次读取的数据（在寄存器中）放到share memory中第二个缓存区。
        s_a[smem_loc_next][load_a_smem_k    ][load_a_smem_m] = r_load_a[0];
        s_a[smem_loc_next][load_a_smem_k + 1][load_a_smem_m] = r_load_a[1];
        s_a[smem_loc_next][load_a_smem_k + 2][load_a_smem_m] = r_load_a[2];
        s_a[smem_loc_next][load_a_smem_k + 3][load_a_smem_m] = r_load_a[3];
        FLOAT4(s_b[smem_loc_next][load_b_smem_k][load_b_smem_n]) = FLOAT4(r_load_b[0]);
...
int main() {
    ...
    {
        const int M = 192, N = 192, K = 512;
        dim3 blockDim(BN / TN, BM / TM);
        dim3 gridDim((N + BN - 1) / BN, (M + BM - 1) / BM);
        float max_error = testError(sgemm_v3, gridDim, blockDim, M, N, K);
        printf("Max Error = %f\n", max_error);
    }
    ...
    for (int i = 0; i < TESTNUM; ++i) {
        ...
        for (int j = 0; j < outer_repeat; ++j) {
            double this_sec = testPerformance(sgemm_v3, gridDim, blockDim, M, N, K, inner_repeat);
